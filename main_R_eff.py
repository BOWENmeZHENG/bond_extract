...
sigmas = [3.0, 5.0, 7.0, 9.0, 11.0, 13.0]
seeds = [12, 13, 14, 15, 16]

for seed in seeds:
    for sigma in sigmas:

        trj_name = f'f_196_i_200_mi_1.0_s_{seed}_sgm_{sigma}_cut_{sigma+3.0}_c_10_t_50000_at_2000'
        os.makedirs(f'analysis/{trj_name}', exist_ok=True)

        steps, box_sizes, xyz = utils.parse(f'trajectories/R_eff/_{trj_name}/{trj_name}.lammpstrj')
