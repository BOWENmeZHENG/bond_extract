    ...
    for prop in property_list:
        for i, num in enumerate(num_cycles):
            ...
            for seed in seed_list:
                trj_name = f'S_3_f_198_i_200_mi_1.0_s_{seed}_sgm_{prop}_cut_{prop+3}_c_10_t_50000_at_2000'
                data = pd.read_csv(f'analysis/S3/{trj_name}/bond_per_atoms_selected.out', header=None)
...
def plot_results(all_results_mean_std):
    ...
    plt.text(0, 1.47, r'$S=3,  \, r=1$', fontsize=20)
    for prop, data in all_results_mean_std.items():
        ...
        np.savetxt(f'saved/S3/sigma_{prop}.x', data[:, 0])
        np.savetxt(f'saved/S3/sigma_{prop}.y', data[:, 1])
        np.savetxt(f'saved/S3/sigma_{prop}.yerr', data[:, 2])
...
example_trj = f'analysis/S3/S_3_f_198_i_200_mi_1.0_s_13_sgm_3_cut_6_c_10_t_50000_at_2000/bond_per_atoms_selected.out'
